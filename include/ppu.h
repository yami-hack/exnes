...
INLINE void exnes_ppu_render_name_table(exnes_t*nes,uint16_t *line_pixel,int line){
    ...
    for(;draw_x<draw_width;draw_x){
        ...
        for(xx=7;xx>=0;xx--){
            ...
            line_pixel[draw_x+xx] = nes->pal[curpal[pixel_data]];
            x++;
        }
        draw_x += 8;
    }
    ...
    for(i=0;i<64;i++){
        ...
        if(y>=0&&y<8){
            ...
            if(is8x16){
                ...
                //类似map
                u8 *tile = nes->vram + ((exnes_ppu2000_t*)(&nes->ppu[0]))->pattab_invram_1000_sprite*0x1000 + tid * 0x10; /*tile*/
                draw_x = x + vflip?0:7;
                tile += y*1;    /*第y行*/
                if(bg_nofront){
                    u32 t1 = tile[0];
